...
- модуль `Квесты и рецепты`: награды, чертежи и маркеры контрактов; конкретные семейства чертежей выбираются отдельно, а метка `[Ч]` зависит от выбранного фильтра;
...
- явное применение в LIVE: preflight SCMDB, backup `global.ini` и краткий боевой рапорт;
...
Это версия `1.0.3` нового продукта. Игрок запускает один проект: `SC Mod Launcher` с набором модулей.
...
Из релизного архива рекомендуется извлекать именно папку `SC_Mod_Launcher`. Внешняя папка, которую Windows создаёт через `Извлечь все`, может называться по версии архива (`SC_Mod_Launcher_1.0.3`) и не должна быть рабочим именем установки.
