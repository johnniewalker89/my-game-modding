...
Это версия `1.0.6` нового продукта. Игрок запускает один проект: `SC Mod Launcher` с набором модулей.
...
Из релизного архива рекомендуется извлекать именно папку `SC_Mod_Launcher`. Внешняя папка, которую Windows создаёт через `Извлечь все`, может называться по версии архива (`SC_Mod_Launcher_1.0.6`) и не должна быть рабочим именем установки.
